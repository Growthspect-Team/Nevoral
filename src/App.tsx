...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/mvzvjggl', {
        method: 'POST',
        body: data,
...
const FloatingActionButton = () => {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.9 }}
      className="fixed bottom-8 right-8 z-[100] w-16 h-16 bg-brand-red text-white rounded-full shadow-2xl flex items-center justify-center md:hidden"
    >
      <Phone size={28} />
    </motion.a>
  );
